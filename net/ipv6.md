...
### 地址分类
1. 单播
    1. 链路本地地址(link-local address), 指在同一个数据链路内唯一的地址
           
           其前缀是`FE80::/64`,即前10b为`1111,1110,10`. 当一个节点启用ipv6协议栈时, 节点的每个接口会自动配置一个该类地址, 使得两个连接到同一链路的ipv6节点不需要做任何配置就可以通信, 即不经过路由器就在同一个链路中通信. 通常接口 ID 保存 64 比特版的 MAC 地址.
    1. 站点本地地址(site-local address)

           类似ipv4的私有地址. 任何没有申请到提供商分配的可聚合全球单播地址的组织机构都可以使用该地址.该地址由3部分组成:
           1. `FEC0::/48`前缀,即前10b为`1111,1110,11`.
           1. 子网ID(16b)
           1. 接口ID(64b)
