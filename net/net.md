...
相比于广播, 播既可以穿透路由器, 可以实现只给那些必要的组发送数据包.
dns是让人使用名称, 让路由器使用 IP 地址进行通信的机制.
DHCP是自动设置 IP 地址、统一管理 IP 地址分配的机制.
NAT用于在本地网络中使用私有地址，在连接互联网时转而使用全局 IP 地址的技术, 是为正在面临地址枯竭的 IPv4 而开发的技术. 不过，在 IPv6 中为了提高网络安全也在使用 NAT.
...
> MAC 头部很容易被误解为是由网卡来处理的, 际上它是由 TCP/IP 软件来负责的.
...
### 路由
该路由控制表的形成方式有两种：一种是管理员手动设置, 一种是路由器与其他路由器相互交换信息时自动刷新(由路由协议构建). 前者也叫做静态路由控制, 后者叫做动态路由控制.

### IP 分包与组包
- 每种数据链路的最大传输单元（MTU）都不尽相同, 为每个不同类型的数据链路的使用目的不同. 使用目的不同, 承载的 MTU 也就不同.
- 任何一台主机都有必要对 IP 分片进行相应的处理. 分片往往在网络上遇到比较大的报文无法一下子发送出去时才会进行处理
- 经过分片之后的 IP 数据报在被重组的时候, 能由目标主机进行. 路由器虽然可做分片但不会进行重组.

#### 路径 MTU 发现
- 分片机制也有它的不足, 如路由器的处理负荷加重之类. 因此, *只要允许是不希望由路由器进行 IP 数据包的分片处理D.
- 为了应对分片机制的不足, 路径 MTU 发现” 技术应运而生. 路径 MTU 指的是, 发送端主机到接收端主机之间不需要分片的最大 MTU, 即路径中存在的所有数据链路中最小的 MTU.
- 进行路径 MTU 发现, 可以避免在中途的路由器上进行分片处理, 可以在 TCP 中发送更大的包.
...
- A类IP 地址是首位以 `0` 开头的地址,从第 1 位到第 8 位是它的网络标识
    用十进制:`0.0.0.0~127.0.0.0` 是 A 类的网络地址, A 类地址的后 24 位相当于主机标识
- B类IP 地址是前两位 `10` 的地址, 从第 1 位到第 16 位是它的网络标识
    用十进制表示`128.0.0.0~191.255.0.0`是 B 类的网络地址, B 类地址的后 16 位相当于主机标识
- C类IP 地址是前三位为 `110` 的地址. 从第 1 位到第 24 位是它的网络标识
    用十进制表示`192.0.0.0~223.255.255.0` 是 C 类的网络地址, C 类地址的后 8 位相当于主机标识
- D类IP 地址是前四位为 `1110` 的地址, 从第 1 位到第 32 位是它的网络标识
    用十进制表示`224.0.0.0~239.255.255.255` 是 D 类的网络地址。D 类地址没有主机标识, 用于多播.
...
> ACK起应答作用, SYN起同步作用
> 在Linux上报文段的最大生存时间通常是30秒, 倍的MSL就是一分钟
> TCP规定, YN报文段（SYN=1的报文段）不能携带数据, 需要消耗掉一个序号
> TCP规定, CK报文段可以携带数据, 是如果不携带数据则不消耗序号
> TCP规定, IN报文段即使不携带数据, 要消耗一个序号
...
1. 第一次握手：建立连接. 
    client发送连接请求报文段, SYN位置为1, equence Number(随机)为x；然后, lient进入SYN_SEND状态, 待服务器的确认
2. 第二次握手：服务器端收到数据包后由标志位SYN=1知道client请求建立连接. 
    服务器收到client的SYN报文段, 要对这个SYN报文段进行确认即设置Acknowledgment Number为x+1(Sequence Number+1)；同时, 己自己还要发送SYN请求信息, SYN位置为1, equence Number(随机)为y；服务器端将上述所有信息放到一个报文段（即SYN+ACK报文段）中, 并发送给client, 时服务器进入SYN_RECV状态；
3. 第三次握手：验证server ack并建立连接. 
    client收到服务器的SYN+ACK报文段, 验证server发回的ACK有效性, 通过后将Acknowledgment Number设置为y+1, 服务器发送ACK报文段, 个报文段发送完毕以后, lient和服务器端都进入ESTABLISHED状态, 成TCP三次握手. 
完成了三次握手, lient和服务器端就可以开始传送数据.
...
1. 第一次分手：主机1（假定主机1为发起方）, 置Sequence Number和Acknowledgment Number, 主机2发送一个FIN报文段(表示没有数据要发送)；此时, 机1进入FIN_WAIT_1状态
2. 第二次分手：主机2收到了主机1发送的FIN报文段, 向主机1回一个ACK报文段, cknowledgment Number为Sequence Number加1, 之后进入CLOSE_WAIT状态；主机1收到ACK进入FIN_WAIT_2状态；主机2告诉主机1, "同意”你的关"(求；
). 第三次分手：主机2向主机1发送FIN报文段, 示这边数据也发完并准备好关闭连接了, 时主机2进入LAST_ACK状态；
4. 第四次分手：主机1收到主机2发送的FIN报文段, 主机2发送ACK报文段, 后主机1进入TIME_WAIT状态；主机2收到主机1的ACK报文段以后, 关闭连接；此时, 机1等待2MSL(Double Maximun Segment Life, 两倍的报文段的最大生存时间)后依然没有收到回复, 证明主机2端已正常关闭, 好, 机1也可以关闭连接了.
至此, CP的四次分手就这么愉快的完成了

> 上方介绍的是一方主动关闭, 一方被动关闭的情况, 际中还会出现双方同时发送FIN+ACK进行主动关闭的情况.
> ![tcp同时关闭](images/net/tcp_close_two.jpg)

为什么建立连接协议是三次握手, 关闭连接却是四次握手呢? **tcp是全双工, 双方都要相互确认终止发送数据**,主机2收到FIN时可能还需要发送一些数据给对方, 这也是主机2的FIN和其响应主机1的FIN的ACK不能同时发送的原因.

为什么要有一个 TIME_WAIT 状态？为什么不能直接到达 CLOSED 状态？
1. 可靠地实现TCP全双工连接的终止
假设没有 TIME_WAIT 这种状态. 现实中, 络环境不是理想的. 在数据包传输的过程中, 免会有一些延时、丢包的情况发生. 如果, 主机1最后一个ACK报文段发出去之后, 于某种原因, 有到达主机2, 样, 机2在超时后, 会向主机1重新发一个 FIN 报文段, 求重传. 由于在主机1端, 接实际上已经断开, 口已经关闭. 那么在主机1收到这个报文段后, 向主机2发送一个 RST 报文(RST表示复位, 来异常的关闭连接). 而主机2收到该报文段后, 认为是错误的, 所期望收到的是ACK报文.

2. 保证让迟来的报文段有足够的时间被处理
同样在不理想的网络环境中, 些包会在网络中有所延迟. 假如没有 TIME_WAIT 这种状态, 关闭连接后并且建立新的连接后, 能会收到该数据包. 由于序号不同, 户端会要求服务端重传数据包, 样, 接就会混乱出错. 而 TIME_WAIT 状态的时间, 般是 2MSL 时间, 且端口没有释放. 这样, 一个连接的报文段有足够的时间被识别或者丢弃, 就不会出现这个问题

为什么A(主机1)要先进入TIME-WAIT状态, 待2MSL时间后才进入CLOSED状态？ 为了保证B(主机2)能收到A的确认应答.  若A发完确认应答后直接进入CLOSED状态, 么如果该应答丢失, 等待超时后就会重新发送连接释放请求, 此时A已经关闭了, 会作出任何响应, 此B永远无法正常关闭.

### 可靠性
网络出现错误时, tcp协议栈会负责重传; 因此网卡、路由器、应用程序都没有错误补偿机制, 旦检测到错误就直接丢弃相应的包. TCP 会在尝试几次重传无效之后强制结束通信, 并向应用程序报错.
...
#### 通过序列号与确认应答提高可靠性即是否收到了网络包
1. 如果有确认应答, 明数据已经成功到达对端; 反之, 数据丢失的可能性很大
1. 在一定时间内没有等待到确认应答, 送端就可以认为数据已经丢失, 进行重发. 由此, 使产生了丢包, 然能够保证数据能够到达对端, 现可靠传输. 
...
1. 对于目标主机来说, 复收到相同的数据是不可取的. 为了对上层应用提供可靠的传输, 标主机必须放弃重复的数据包. 为此我们引入了序列号. 
1. 序列号是按照顺序给发送的数据包都标上号码的编号. 接收端查询接收数据 TCP 首部中的序列号和数据的长度, 自己下一步应该接收的序列号作为确认应答返送回去. 通过序列号和确认应答号, CP 能够识别是否已经接收数据, 能够判断是否需要接收, 而实现可靠传输

![序列号和确认应答](images/net/sequence_ack.jpg)

#### 重发超时的确定
1. 重发超时是指在重发数据之前, 待确认应答到来的那个特定时间间隔. 如果超过这个时间仍未收到确认应答, 送端将进行数据重发. 最理想的是, 到一个最小时间, 能保证"确认应答一定"(这个时)内返回”. 
1. TCP 要求不论处在何种网络环境下都要提供高性能通信, 且无论网络拥堵情况发生何种变化, 必须保持这一特性. 为此, 在每次发包时都会计算往返时间及其偏差. 将这个往返时间和偏差时间相加, 发超时的时间就是比这个总和要稍大一点的值. 
1. 在 BSD 的 Unix 以及 Windows 系统中, 时都以0.5秒为单位进行控制, 此重发超时都是0.5秒的整数倍. 不过, 初其重发超时的默认值一般设置为6秒左右. 
1. 数据被重发之后若还是收不到确认应答, 进行再次发送. 此时, 待确认应答的时间将会以2倍、4倍的指数函数延长. 
1. 此外, 据也不会被无限、反复地重发. 达到一定重发次数之后, 果仍没有任何确认应答返回, 会判断为网络或对端主机发生了异常, 制关闭连接. 并且通知应用通信异常强行终止. 

#### 以段为单位发送数据
1. 在建立 TCP 连接的同时, 可以确定发送数据包的单位, 们也可以称其为"最大消息长度"(MSS). 最理想的情况是, 大消息长度正好是 IP 中不会被分片处理的最大数据长度. 
1. TCP 在传送大量数据时, 以 MSS 的大小将数据进行分割发送. 进行重发时也是以 MSS 为单位. 
1. MSS 在三次握手的时候, 两端主机之间被计算得出. 两端的主机在发出建立连接的请求时, 在 TCP 首部中写入 MSS 选项, 诉对方自己的接口能够适应的 MSS 的大小. 然后会在两者之间选择一个较小的值投入使用
...
![滑动窗口控制](images/net/tcp_window/tcp_window_move.jpg)
