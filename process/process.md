...
1. 用户态(1:M) : java
   ...
   将多个用户态线程映射给单一的内核态线程. 着多核的普及, 用户态线程的数量也在不断增加, 多对一模型难以适应这种变化, 因而目前在主流操作系统中已经不再使用了. 但随着新背景(用户态对线程的语义和执行状态更加了解, 可实现更优的调度决策)的出现, os提供了纤程（fiber）的支持即ucontext, 用户态线程和内杉态线程的关系也由一对一向多对一扩展.
