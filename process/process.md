...
> 调用fork后父子进程的状态存在大量共享,因此会造成很多不确定的行为. POSⅨ的标准中列出了调用fork时的25种特殊情况的处理方法, 其中既包含进程标识符和文件, 还包含锁、计时器、消息队列等.

> linux针对fork缺点, 提出了针对fork的多种替代方案, 包括spawn、vfork、clone等.

> Windows采用的进程创建接口为CreateProcess系列, 与fork相比CreateProcess的设计更加直观, 它会从头创建进程, 同时它需要对进程的运行参数进行大量配置, 因此在提供灵活性的同时也使接口变得异常复杂.
...
1. 用户态线程（user-level thread）
	...
	应用自己创建的, 内核不可见, 因此不直接受系统调度器管理
2. 内核态线程（kemelˉlevel thread）
...
多线程模型（multithreading model）:
1. 用户态(1:M) : java

   将多个用户态线程映射给单一的内核态线程. 随着多核的普及, 用户态线程的数量也在不断增加, 多对一模型难以适应这种变化, 因而目前在主流操作系统中已经不再使用了. 但随着新背景(用户态对线程的语义和执行状态更加了解, 可实现更优的调度决策)的出现, os提供了纤程（fiber）的支持即ucontext, 用户态线程和内杉态线程的关系也由一对一向多对一扩展.

   > 为与os提供的fiber区别, 编程语言提供的fiber支持称为协程（coroutine）.

   不需要内核支持而在用户程序中实现的线程, 其不依赖于操作系统核心，应用进程利用线程库提供创建、同步、调度和管理线程的函数来控制用户线程.

   优点: 线程切换简单, 进程可以有自定义的调度算法.
   缺点: 操作系统内核不知道多线程的存在，因此一个线程阻塞将使得整个进程（包括它的所有线程）阻塞, 即用户级线程是OS内核不可感知的; 用户级线程不能利用系统的多核处理，仅有一个用户级线程可以被执行, 其他用户态线程被阻塞
1. 内核级(1:1)

   由操作系统内核创建和撤销, 内核维护进程及线程的上下文信息以及线程切换.一个内核线程由于I/O操作而阻塞，不会影响其它线程的运行.

   优点: 可以利用多核
   缺点: 线程在用户态的运行，而线程的调度和管理在内核实现，在控制权从一个线程传送到另一个线程需要用户态到内核态再到用户态的模式切换，比较占用系统资源. 同时由于每
个用户态线程都对应于一个内核态线程, 创建内核态线程的开销会随着用户态线程数量的增加而不断增大.

	linux和Windows都采用的是一对一模型.

1. 混合式(M:N) : go的GPM模型

   内核只识别内核级线程, 其再被用户级线程多路复用. 既减轻了多对一模型中因为内核态线程过少而导致的阻塞问题, 也解决了一对一模型中因为内核态线程过多而导致的性能开销过大的问题

   但多对多模型也会让内核态线程的管理变得复杂. Solaris在版本9之前都提供多对多的线程模型, 但在版本9之后改为一对一模型. macOS和iOS使用的面向用户体验的调度器GCD也采用了多对多模型.
...
1. 分离的内核栈与用户栈
	...
	由于每个线程的执行相对独立, 进程为每个线程都准备了不同的栈, 供它们存放临时数据.
1. 共享的其他区域:进程除栈以外的其他区域由该进程的所有线程共享,包括**堆**、数据段、代码段等

	因为在同一个堆上分配内存, 因此malloc的实现需要使用同步原语.
...
## 上下文(context)
执行上下文(execution context)又称为进程状态(process state), 是os用来管理和控制进程所需的内部数据, 其能够用于保存和恢复进程在处理器上运行的状态. 可简单理解为被运行中的进程加载到寄存器的数据集.
...
### 线程实现
与进程类似, 线程也有自己的线程控制块（Thread Control Block, TCB）用于保存与自身相关的信息. 在目前主流的一对一线程模型中, 内核态线程和用户态线程会各自保存自己的TCB. 对于Linux平台上使用pthread线程库的应用来说, pthread结构体就是用户态的TCB. 用户态的TCB可以认为是内核态的扩展, 可以用来存储更多与用户态相关的信息, 比如线程本地存储（Thread Local Storage,TLS）.

> 由于TLS结构的相似性, 对TLS中变量寻址的实现方式也较为特殊. x86-64使用了FS段寄存器来实现TLS中变量的寻址. 类似地, 在AArch64中, TLS的起始地址将被存放在寄存器TPIDR_EL0中.
...
pthreads接口:
- pthread_create: 线程创建, 它基于clone实现的
- pthread_exit: 线程退出
- pthread_yield: 出让资源

	与sleep最重要的区别是调用后线程的状态: 当线程调用yield之后,它仍会处于预备状态, 并可能很快就会被调度

	yield适合于等待条件很快就会被满足的情况; sleep由于涉及线程的状态的变化, 适合于等待条件被满足的时间不确定或较长的情况

- pthread_join: 提供合并（join）操作, 允许一个线程等待另一个线程的执行, 并获取其执行结果
- pthread_cond_wait: 在进人挂起状态之前指明等待的具体事件
- pthread_cond_timedwait: 相当于"等待具体事件"和"等待固定时间"的结合. pthread_cond_timedwait还接收一个额外的时间参数abstime， 如果等待时间超过abstime或cond被其他线程使用pthread_cond_signal操作, 内核都会将其唤醒
...
默认情况下, 父进程和子进程属于同一个进程组. 如果子进程想要脱离当前进程组, 可以通过调用setpgid创建一个新的进程组或者移入已有的进程组. 进程组的一大作用体现在对信号的处理上. 应用程序可以调用killpg向一个进程组发送信号, 这个信号会被发送给这个进程组的每个进程.
...
## 僵尸进程(Zombie)
**子进程先于父进程结束**，但是其父进程尚未对其进行善后处理（获取已终止子进程的有关信息、释放相关资源, 核心是处理子进程的PCB）的进程.

查看方法: top中的`zombie`数量. 定位僵尸进程以及该僵尸进程的父进程: `ps -A -ostat,ppid,pid,cmd |grep -e '^[Zz]'`
...
> 使用文件描述符而非直接指向文件系统中的某个文件, 是因为文件结构可能根据不同的文件系统而变化, 提供单一抽象有利于操作系统统一管理.
