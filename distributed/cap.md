...
- 网络分区：俗称”脑裂”，部分节点和整个分布式系统失去联系，自己单独组成了一个小集群
    ...
    1. 主动stepDown：Leader无法联系到过半数节点时主动放弃领导权
    1. 严格的投票规则：每个term每个节点只能投票给一个候选人

- 三态：成功、失败和超时. 无法预测超时的请求是否到达了接收方，还是在接收方返回的时候丢失了

![导图](/misc/pdf/distributed/分布式协议与算法.pdf)

> 共识（Consensus）与一致性（Consistency）的区别：一致性是指数据不同副本之间的差异，而共识是指达成一致性的方法与过程.

## 两将军问题和拜占庭将军问题(The Byzantine Generals Problem, BYP)
> 拜占庭将军问题是由莱斯利·兰波特(Leslie Lamport)在其同名论文中提出的分布式对等网络通信容错问题

两将军问题讨论的是分布式系统中网络通信上的原因，导致系统会出现的问题即网络的不确定性；而拜占庭将军问题，则讨论的是分布式系统中的节点会出现的问题即节点的不可信性.
...
口信消息型拜占庭问题解决方法: 如果叛将人数为 m, 将军人数不能少于 3m + 1, m决定通信轮次(递归循环次数)至少m+1 <=> n位将军, 做多能容忍(n-1)/3位叛将

> 通信轮次: 自身作为司令官, 向其他未向自己发送过信息的将军们发送信息
> 通信目的: 确保所有忠诚的将军在同一个计划上达成共识, 而不是揪出所有叛徒
> m+1: 至少会包含一个忠诚的节点, 这个忠诚的节点会如实广播它收到的信息，从而打破叛徒的信息封锁，确保最终的多数派投票能够得出正确的结论

签名消息型拜占庭问题解决方法: 通过签名机制约束叛将的叛变行为，任何叛变行为都会被发现，也就会实现无论有多少忠诚的将军和多少叛将，忠诚的将军们总能达成一致的作战计划
...
## 一致性模型
一致性模型: 在并发编程中, 系统和开发者之间的一种约定, 如果开发者遵循某些规则, 那么开发者执行读/写操作的结果是可预测的. 即在分布式系统中的读写请求，表现得像在单机系统上一样，符合直觉和预期. 它本质上定义了写操作的顺序和可见性, 即并发写操作执行顺序是如何, 写操作的结果何时能被别的进程看到.
...
FLP **定理**实际上说明了在允许节点失效的场景下,基于**异步通信**方式的分布式协议,无法确保在有限的时间内达成一致性, 即在异步系统中，共识算法无法同时满足安全性（Safety，即一致性）、活性（Liveness，即终止性）和容错性.

> FLP 不可能原理告诉我们: 不要浪费时间，去试图为异步分布式系统设计面向任意场景的共识算法.
>
> 在分布式系统中,“异步通信”与“同步通信”的最大区别是没有时钟、不能时间同步、不能使用超时、不能探测失败、消息可任意延迟、消息可乱序等.

根据 FLP 定理,实际的一致性协议( Paxos 、 Raft等)在理论上都是有缺陷的, 最大的问题是理论上存在不可终止性! 至于 Paxos 和 Raft 协议在工程的实现上都做了调整(例如, Paxos 和 Raft 都通过随机的方式显著降低了发生算法无法终止的概率), 并使用同步假设(或保证 safety ,或保证 liveness ), 从而规避了理论上存在的问题.
