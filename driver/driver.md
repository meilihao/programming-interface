...
## 地址
Linux涉及三种地址:
1. 物理地址(PA,PhysicalAddress)

	CPU地址总线使用的地址, 由硬件电路控制其具体含义. 物理地址中很大一部分是留给内存的, 但也常被映射到其他存储器上（如显存、BIOS等）.

1. 线性地址(LA,LinearAddress)

	虚拟地址经过分段处理变成了线性地址,在使能了内存管理单元(MMU)的系统中,线性地址经过转换生成物理地址(常说的分页), 否则线性地址就被直接当作物理地址

1. 虚拟地址(VA,Virtual Address)

	应用程序中使用的是虚拟地址.

	在程序指令中的虚拟地址经过段映射和页面映射后, 就转换成了物理地址.
...
分段机制完成了虚拟内存到线性内存的映射, Linux对分段的支持非常有限, 同时现代cpu都推荐使用分页.

Linux主要的四个段, 它们的段描述符表示的基准地址全部为0,因此转化后的线性地址与虚拟地址相等. 它们分别为:
1. 用户代码段(`__USER_CS`)
1. 用户数据段(`__USER_DS`)
1. 内核代码段(`__KERNEL_CS`)
1. 内核数据段(`__KERNEL_DS`)
...
线性地址空间的页是逻辑上的页,称之为页表(PageTable),是虚拟的;物理空间的页,称之为页框(PageFrame),是真实存在的,只不过被逻辑上划分为一页页,页的大小一般为4K(2M、4M和1G也
有可能). 分页就是将二者对应起来的过程,分为两部分,一是从页表上的地址找到页框上的真实地址(寻址),二是建立页表和页框的映射关系.

### 寻址
线性地址由硬件MMU转换成物理地址. 线性地址本质上就是偏移量.

将地址分为多级来解释的目的是节省内存: 分级的好处是可以只映射一部分需要使用的内存,减少项占用的内存.

> X86还有其他的分页方式, 如PSE(PageSizeExtension), 它注定被PAE取代.

硬件上可以有多种分页方式,32位的地址可以分成多种级别来解释. Linux为了兼容32位和64位的平台和各种分页方式,3.10版内核统一将线性地址分成 4 个级别来解释,分别叫作页全局目录
(PageGlobalDirectory,PGD)、页上级目录(PageUpperDirectory, PUD ) 、页中级目录(PageMiddleDirectory,PMD)和页表(PageTable,PT),5.05版内核在PGD和PUD之间添加了P4D,总共5级。P4D中的“4”是“4th”的意思.
...
- 内存映射(memory-mapped, 统一编址, mmio): 此类cpu通常只实现一个物理地址空间, 会将外设 I/O端口的物理地址映射到CPU的单一物理地址空间中, 成为存储空间的一部分，通过直接读写内存地址的方式来访问外设, 简化程序设计

	> 外设的MMIO地址经过页表后x86还是会翻译成I/O地址, 最后由(集成的)北桥完成转发; 其他平台的处理方式未知.
	...
	映射的结果就是物理内存区间[res->start, res->end)映射到了虚拟内存区间[my_data->iobase, my_data->iobase +SIZE) 上.

	> 为了程序的可移植性,不要将映射后的地址直接当作指针使用,内核提供`__raw_readl和__raw_writel`封装了不同平台的操作
...
- io映射(i/o-mapped, 独立编址): 为外设专门实现了一个单独地地址空间, 称为`I/O地址空间或I/O端口空间`(32位X86有64K的IO空间, `cat /proc/ioport`). IO地址与内存地址分开独立编址, I/O端口地址不占用存储空间的地址范围. 此时在系统中就存在了另一种与存储地址无关的IO地址. 此时, 需要使用专用的CPU指令来访问某种特定外设, 比如x86的in/out等指令.
	...
	> I/O地址和内存可能会重合. in/out等指令会让cpu知道操作地址是外设的, 再由(集成的)北桥转发
