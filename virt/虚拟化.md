...
- [**虚拟化分类与 I/O 虚拟化技术概述**](https://cloud.tencent.com/developer/article/2327479?areaId=106001)
...
提供虚拟化的软件层称为 Hypervisor或vmm(virtual machine monitor, 虚拟机监视器). VMM对物理资源的虚拟归纳为三个部分:
- CPU虚拟化

  虚拟机提供虚拟处理器（virtual CPU, vCPU）的抽象并执行其指令的过程
- 内存虚拟化
  ...
  为满足上述需求, 产生了客户物理地址（Guest Physical Address, GPA）.

  客户虚拟地址转换为客户物理地址的过程被称为第一阶段地址翻译; 客户物理地址转换为主机物理地址的过程被称为第二阶段地址翻译.

  arm使用了直接页表映射（direct paging）机制. 在ARM的硬件虚拟化拓展中, 硬件在EL2特权级添加了第二阶段页表（stage-2 page table）. 此页表记录着客户物理地址到主机物理地址的映射关系. 原先虚拟机内使用的页表被称为第一阶段页表（stage-1 page table）, 它记录着客户虚拟地址到客户物理地址的映射关系.
- IO虚拟化
  ...
  1. 软件模拟（全虚拟化）的方法
    ...
    以网卡举例, 它需要使用设备提供的MMIO接口, 不仅客户操作系统每次读写MMIO都会造成虚拟机下陷, 而且多次MMIO访问只能传输少量的数据（例如一个网络包）.

  1. 半虚拟化的方法

    以网卡举例, 多次的网络包数据可被同时放置于共享内存中, 而这些数据的传输只需使用一次超级调用. 通过这种批量处理的方式减少了虚拟机下陷的次数, 提高了I/O虚拟化的性能. 

  1. 设备直通的方法.

    尽管半虚拟化的方法已经具备较好的性能, 但因为在数据传输过程中偶尔会发生虚拟机下陷（由超级调用引起）, 它的性能依然无法与直接使用物理设备相比.

    设备直通意味着让物理设备从虚拟机监控器的管理中脱离,并将其直接交与虚拟机中的原生驱动程序进行管理. 这种虚拟化方法的优势显而易见:虚拟机在使用设备时不需要任何虚拟机监控器的参与, 所以与软件模拟或半虚拟化的方法相比, 该方法大大提升了I/O虚拟化的性能.

    > 由于DMA攻击属于一种通用类型的问题, 几乎所有的厂商都提供了相对应的IOMMU实现方案以解决该问题, 例如Intel的VT-d、AMD的AMD-Vi、ARM的SMMU等.

    由于I/O过程完全绕开了VMM, 其无法在任意时刻检查虚拟机的状态, 因而难以支持虚拟机热迁移以及虚拟机安全自省等功能.

    ![](/misc/img/virt/io_virt_arch.png)
...
## 特权指令和敏感指令
特权指令是OS中直接管理关键系统资源的指令, 指在用户态执行时会触发下陷的指令, 包括系统调用和不允许石用户态执行的指令（例如写入只读内存）等
敏感指令是虚拟机中操作特权资源的指令.

  敏感指令通常包括以下类型:
  - 读写特殊寄存器或执行特殊指令以更改CPU状态, 比如x86架构中修改CR0或CR4寄存器, 在ARM架构中修改SCTRL_EL1寄存器
  - 读写敏感的内存, 例如读写未映射的内存、写入只读页等
  - 执行I/O指令, 比如x86架构中的in和out指令等

特权指令都是敏感指令，但是敏感指令不一定是特权指令.

“陷入再模拟”其实就是VMM捕获所有敏感指令，VMM模拟执行引起异常的敏感指令.

判断一个系统是否可虚拟化，核心就在于系统对敏感指令的支持:
- 如果所有的敏感指令都是特权指令, 即所有的敏感指令在非特权级执行时都会触发下陷, 则可虚拟化
- 不可虚拟化导致敏感指令在用户态执行时不会触发任何下陷, 这也就意味着这些指令不会被VMM捕捉到.
...
1. 硬件虚拟化技术
  ...
  硬件虚拟化对敏感指令的支持主要体现在两个方面:
  1. 一部分的敏感指令不再需要下陷, 而是被直接重定向到了虚拟硬件部件
  1. 过去不可虚拟化架构中的敏感指令也都可以通过配置的方式引发下陷, 从而能够被虚拟机监控器处理

  > ARMv8.0 引入了一个全新的特权级--Exception Level2（EL2）, 它是除了EL3之外最高的特权级. 在此特权级中运行的是VMM, 它控制所有的物理资源, 并管理运行在非特权级(也就是运行在EL0和EL1)中的软件. 实际该特权级中只能运行Type-1的虚拟机监控器, 不支持Type-2的虚拟机监控器, 如KVM. 因此在ARMv8.0的硬件虚拟化中, 必须将KVM的一部分功能从Linux中剥离出来, 以Lowvisor的形式运行在EL2之中. 宿主操作系统Linux和其他KVM功能则依然运行在EL1之中, 该架构设计造成了KVM和Lowvisor之间的多次特权级切换, 因而对虚拟机的运行性能带来了一定的开销. ARMv8.1中进一步扩展了硬件虚拟化的功能, 并增加了Virtualization Host Extension（VHE）, 其意味着可在EL2中运行完整的宿主操作系统. 因此, 在ARMv8.1之后, KVM和宿主操作系统Linux之间的交互不会带来任何特权级切换, 从而优化了虚拟化的性能.

  在引入硬件虚拟化技术后, 虚拟机可直接使用CPU中的寄存器, 而无须使用软件模拟的寄存器.

  在ARM硬件虚拟化中, 硬件为ELI和EL2提供了两套系统寄存器. 这也意味着, 如果发生虚拟机下陷, 虚拟机监控器无须保存虚拟机在EL1中使用的系统寄存器. 此外, 运行在EL2中的虚拟机监控器可以读写EL1和EL0中的任何寄存器. 因而在处理虚拟机下陷时, 它可以随时查看和改变虚拟机的寄存器状态. 但在发生vCPU上下文切换时, 虚拟机监控器依然需要将该vCPU在EL0和EL1中的所有系统寄存器以及通用寄存器都保存在内存之中, 并将即将运行的vCPU的相关寄存器加载至物理寄存器之中.

  在x86中, 每当虚拟机执行敏感指令时, 会触发虚拟机下陷, CPU模式随之由非根模式切换为根模式. 与ARM硬件虚拟化不同的是: x86CPU中的不同特权级共享同一份系统寄存器. 因此, 当触发了虚拟机下陷时, 硬件会将vCPU的所有系统寄存器（例如CR0, CR3等）保存到VMCS之中.
...
1. 物理中断
  ...
  物理中断由物理设备直接产生, 交给虚拟机监控器处理
1. 虚拟中断

  虚拟中断不由物理设备直接产生,而由虚拟机监控器（或ITS）产生并传递给虚拟机.

在硬件虚拟化技术被推出之前, 中断虚拟化需通过软件模拟的方式实现. 一方面, 软件模拟方法中的所有中断寄存器本质上都是内存中的状态, 这些虚拟寄存器的状态改变需要由虚拟机监控器维护. 另方面,在插人虚拟中断时, 虚拟机监控器直接调用虑拟机内的中断处理函数. 通过软件模拟的方法实现中断虚拟化会带来崩多的虚拟机下陷因而性能较差.

arm GIC v2添加中断虚拟化的硬件支持后, 虚拟机监控器可以为虚拟机配置硬件实现的虚拟寄存器, 虚拟机可使用这些寄存器控制中断的行为（例如中断的开关）. 虚拟机在访问这些寄存器时不会产生任何虚拟机下陷. 此外, 插人中断的方式也得到了优化. 虚拟机监控器可以使用GIC提供的接口插人中断, 此中断将在CPU恢复虚拟机执行时由硬件自动插人并直接调用对应的中断处理函数.

上述中断虚拟化方法虽然已经大大提升了虚拟机在处理中断时的性能, 但每次触发物理中断时依然会引起虚拟机下陷. 尤其是当设备已经直通给虚拟机时, 由物理中断产生的下陷会对虚拟机的I/O性能产生较大的影响. 为了解决该问题, 不同体系结构都推出了新的中断虚拟化技术: 物理设备可以直接向虚拟机发送物理中断（不产生虚拟机下陷). 比如Intel的Posted Interrupt, 它的转发机制主要由IOMMU实现; AArch64是由GIC的ITS(Interrupt Translation Service)实现.
...
- [SR-IOV](https://tonydeng.github.io/sdn-handbook/linux/sr-iov.html)

  - SR-IOV vs PCI path-through
  - Virtio 和 Pass-Through 的详细比较
  - SR-IOV vs DPDK
  - SR-IOV使用示例
- [Scalable IOV技术详解](http://m.blog.chinaunix.net/uid-28541347-id-5854292.html)

  intel 的VT-d解决了设备直通问题，使虚拟机可以直接访问硬件设备从而提高性能，而SR-IOV则提供了设备共享的能力，通过将设备硬件虚拟化出多个VF给不同的虚拟机使用.
...
SR-IOV是一个将PCIe共享给虚拟机的标准，通过为虚拟机提供独立的内存空间、中断、DMA流，来绕过VMM实现数据访问, 即可让os将一个设备识别为多个虚拟设备的功能.

SR-IOV基于两种PCIe functions:
- PF (Physical Function)： 以往以物理方式存在的设备, 包含完整的PCIe功能, 包括SR-IOV的扩张能力，该功能用于SR-IOV的配置和管理
- FV (Virtual Function)： 通过SR-IOV添加的虚拟设备成为VF, 包含轻量级的PCIe功能. 每一个VF有它自己独享的PCI配置区域，并且可能与其他VF共享着同一个物理资源
...
SR-IOV要求:
- CPU 必须支持IOMMU（比如英特尔的VT-d 或者AMD的 AMD-Vi，Power8 处理器默认支持IOMMU）
...
## kvm生态组成
KVM实际上是硬件虚拟化机制的驱动程序, 作为驱动程序, KVM的实现分为两个部分:
1. 向用户态提供驱动API的支持, 允许用户态程序（如QEMU）管理虚拟机相关的状态
2. 行虚拟机并处理虚拟机下陷
...
1. 支持详细配置的`-drive option[,option[,option[,...]]]`参数, 它定义一个存储驱动器
  ...
  - cache=cache
    ...
    ![](/misc/img/virt/20940095_1350022176aC72.png)
...
### 内存气球
在客户操作系统内核中插入一个伪装的驱动, 该驱动不会为内核其他模块提供任何有意义的功能, 它的实际目的是和虚拟机监控器互相配合, 并根据虚拟机监控器的要求, 不断地使用客户操作系统内核的接口分配或释放内存.
