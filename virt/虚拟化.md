...
1. Physical Function（PF， 物理功能） ： 拥有包含SR-IOV扩展能力在内的所有完整的PCI-e功能， 其中SR-IOV能力使PF可以配置和管理SR-IOV功能. 简言之， PF就是一个
...
在操作系统方面， 拥有较新内核的Linux系统（如RHEL 7） 等已经支持CPU和内存的热插拔， 在其内核配置文件与CPU热插拔有关的项:
```conf
...
qemu使用`-smp <n>,maxvcpus=<N>`即可开启cpu热插拔. 在guest中可用`ls /sys/devices/system/cpu/cpu<N>/online`查看cpu状态.
