...
在编译阶段, 编译器是无法确定每个符号的最终链接地址, 因此编译阶段生成的可重定位目标文件中所有的符号都暂设定为0x0, 链接器在链接最终可执行二进制文件时才具有全局内存地址布局,  这些符号的最终地址由链接器来分配和确定, 这个过程称为链接重定位(link relocation)

确定程序运行时地址的过程就是重定位(Relocation), 每个要被修正的地方叫一个重定位入口(Relocation Entry).
...
1. 绝对加载
  ...
  程序在编译或汇编时生成绝对地址的目标代码, 被载入时总被加载到虚拟内存的固定位置, 该方法不利于程序共享内存.
  
  程序的变化也会导致地址的变化, 可用符号引用来解决.

1. 可重定位加载

  编译器或汇编器不使用绝对内存地址, 编译、链接后的装入模块的地址都是从0开始的，指令中使用的地址、数据存放的地址都是相对于起始地址而言的逻辑地址.

  装入时对地址进行`重定位`，将逻辑地址变换为物理地址（地址变换是在装入时一次完成的）.
1. 动态运行时加载

  编译、链接后的装入模块的地址都是从0开始. 被加载内容的所有内存访问都以相对形式表示，并不会立即把逻辑地址转换为物理地址，而是把地址转换推迟到仅在指令运行时才计算绝对地址, 为不影响性能, 有硬件完成.

## 链接
ref:
- [riscv Relocation types见`Table 3. Relocation types`](https://github.com/riscv-non-isa/riscv-elf-psabi-doc/blob/301374e92976e298e676e7129a6212926b2299ce/riscv-elf.adoc#reloc-table)

链接的目的是把多个可重定位目标文件链接成一个统一的可执行二进制文件, 这个过程包括地址和空间分配, 符号解析, 重定位等.

在elf文件规范里定义了一个重定位表(relocation table), 需要重定位的符号存储在重定位段(`.rela.text`)里

> 链接最终结果包含的符号表`.symtab`里保存了各符号最终的地址
...
### 链接过程
链接的过程是由一个链接脚本（Linker Script）控制的，链接脚本描述输出文件的内存布局, 决定了给每个段分配什么地址，如何对齐，哪个段在前，哪个段在后，哪些段合并到同一个Segment，另外链接脚本还要插入一些符号到最终生成的文件中，例如__bss_start、_edata、_end等. 如果用ld做链接时没有用-T选项指定链接脚本，则使用ld的默认链接脚本，默认链接脚本可以用`ld --verbose`命令查看.

SECTIONS描述了如何把输入文件的段映射到输出文件的各个段
...
通过设置`env LD_DEBUG=[all|libs]`并执行程序, 会输出ld log, 可定位到具体so文件, 再通过原文件或`/var/lib/dpkg/info/${package}.md5sums`中的校验值比对, 检查文件是否被改动. 此时使用`readelf -a bad.so`可能会得到错误信息, 比如`bad symbol index: ${addr} in relocreadelf`.

> `env LD_DEBUG=help`会输出LD_DEBUG支持的选项.

> 类似的有`sudo QT_DEBUG_PLUGINS=1 dde-control-center --show`

### 链接器松弛优化(linker relaxation optimization)
在链接阶段的一种优化技术, 旨在减少不必要的指令.

通常精简指令处理器体系结构中需要两条指令来实现对一个符号地址的访问, 一条指令处理符号地址的高位部分, 另一条指令处理符号地址的低位部分. 但在链接阶段, 通常可以使用一条指令完成该流程从而达到优化目的, 这就是链接器松弛优化的作用.

对于riscv, 链接器松弛优化主要涉及两方面:
1. 函数跳转优化

  在编译阶段, 编译器无法确定函数的最终地址, 因此编译器默认把CALL指令解析为AUIPC和JALR. 在链接阶段, 链接器分配和确定里函数的地址, 从而可以根据偏移量是否可以选择使用短跳转指令来实现, 从而达到优化的目的

1. 符号地址访问优化

  链接器在链接阶段以GP寄存器的值为基地址进行寻址, 把AUIPC和ADDI指令的组合给优化成一条ADDI指令.

```bash
# riscv64-linux-gnu-gcc test.c -o test --save-temps -mno-relax # --save-temps: 保留编译过程中所有产生的中间文件. -mno-relax: 关闭链接器松弛优化, gcc默认是开启的(即使用了`-mrelax`)
```

查看中间文件xxx.o, 会发现`R_RISCV_RELAX`, 它们告诉链接器, 在链接阶段可以进行链接器松弛优化