...
1. slub负责分配比页更小的内存对象, 解决内部碎片的问题
	...
	判断系统是否在用slub的方法:
	1. 看是否存在/sys/kernel/slab目录，存在即使用slub，没有就是slab
	1. `cat /proc/slabinfo | head -n 1`

		- 如果第一行包含 slabinfo → SLAB
		- 如果第一行包含 slabinfo - version: 2.1 → SLUB
	1. `dmesg | grep -i slub`
...
TLB是特别的cpu缓存, 是MMU的核心部件, 通过缓存进程最近使用的分页映射来加速地址转换(虚拟地址 -> 物理地址), 因此被称为快表. 本质上就是一块高速缓存（Cache），缓存了虚拟页号到物理页号的映射关系.
...
## 缺页异常
常见的页缺失有下面这两种：
1. 硬性页缺失（Hard Page Fault）：物理内存中没有对应的物理页. Page Fault Handler 会指示 CPU 从已经打开的磁盘文件中读取相应的内容到物理内存，而后交由 MMU 建立相应的虚拟页和物理页的映射关系
1. 软性页缺失（Soft Page Fault）：物理内存中有对应的物理页，但虚拟页还未和物理页建立映射. Page Fault Handler 会指示 MMU 建立相应的虚拟页和物理页的映射关系
