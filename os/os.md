...
- 软件
  ...
  - 系统软件: 用于构建控制底层计算机硬件的软件, 比如os, 编译器等
  - 应用软件
...
os: 为上层软件提供抽象, 管理和控制计算机硬件与软件资源的系统软件, 包括bootloader, 内核层和应用层:
- 引导启动
...
- 内核层: 
  ...
  - 异常/中断处理
    ...
    异常即程序运行时的错误, 比如除零, 段溢出/段错误(segmentation fault), 页错误, 无效指令, 调试错误等, 需根据错误类型和程序逻辑进行相应的处理.

    异常可以进一步分为错误Fault、陷阱Trap和终止Abort:
    - 错误：错误是系统可以纠正的异常

      例如当一个进程尝试访问某个已经被换出到硬盘的页时, 当请求的地址在进程的地址空间中，并且满足访问权限时，如果页不在内存（RAM）中，将会产生一个中断请求（IRQ），并开始启用缺页异常处理程序把所需的页加载到内存中; 如果操作成功执行，程序将继续运行.
    - 陷阱：陷阱主要用在调试中

      如果在某个程序中设置了一个断点，就插入了一条可以触发陷阱执行的特殊指令. 陷阱可以触发上下文切换来允许调试器读取和展示局部变量的值, 之后程序可以继续运行. 陷阱同样也是运行系统调用的方式（如杀死一个进程）.
    - 终止：终止是由系统表中的硬件错误或值不一致而导致的

      终止不会报告造成异常的指令的所在位置. 这是最严重的中断，终止将会调用系统的终止异常处理程序来结束造成异常的进程.
    
    中断处理是处理器收到硬件设备发出/软件模拟的中断请求并进行相应的处理, **处理结束后再返回到现行程序的中断处，继续执行原程序**, 相关效率会影响系统整体性能. 通常，中断处理会被分为中断上半部和中断下半部: 中断上半部要求快速响应中断，在取得必要的数据和信息后尽早开启中断，以使处理器能够再次接收中断请求信号; 中断下半部被用来执行剩余中断内容, 像数据解析、驱动程序状态调整等更耗时的内容. 为了让更紧迫的进程优先执行，中断下半部还可将处理内容安放在一个进程中，以让更高优先级的进程得到快速执行.

    中断向量（Interrupt vector, 向量即指针）是中断服务程序的入口地址.
    中断处理是优先级最高的任务之一, 其通常由io设备产生. 中断也意味着上下文切换.

    > 系统调用作为一种特殊的中断，它的执行过程可以类推到其他中断信号触发的中断服务处理过程.
    ...
    同时处理多个中断的方法:
    1. 正在处理一个中断时, 禁止再发生中断(即忽略). 缺点: 未考虑相对优先级和时间限制的要求
    1. 定义中断优先级, 实现中断嵌套(**推荐**).
...
死锁: 一组相互竞争系统资源或进行通信的进程间的"永久"阻塞, 它的三个必要条件是: 1. 互斥; 2. 资源被占有只能等待 3. 不可抢占.
可重用资源: 一次仅供一个进程安全使用且不因使用而耗尽的资源.

> 所有intel cpu在底层同步中使用XCHG指令.
...
执行i/o的三种方法:
1. 可编程(programmed)i/o : cpu需不断询问i/o模块的状态是否已完成或可接收新req.
1. 中断驱动i/o: 由io模块发送中断通知cpu自身状态, 比可编程io更高效, 但仍需要cpu干预在存储器和io模块间的数据传输.
1. 直接内存存取(DMA): 由系统总线的一个独立模块(DMA模块)完成. cpu发送具体参数给dma模块后甩手不管, 由dma模块与存储器交互, 完成数据传输后dma模块给cpu发送中断通知, 即cpu仅参与数据传输的开始和结束部分. dma模块会使用总线与存储器传输数据, 如果cpu此时需要使用总线就要等待, 尽管如此, dma仍比中断驱动io高效.

> dma与总线有多种组织方式, 有微小的差异.

## FAQ
### 用户态切换到内核态的 3 种方式: 中断、异常和系统调用
