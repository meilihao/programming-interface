...
实现io(在 CPU和 I/O 之间传输数据)的三种方式:
1. 发起syscall再循环检测io是否完成

   CPU 通过在程序中循环查询指定设备控制器中的状态来判断是否可以与设备进行数据交换.

   编程简单, 但费cpu时间
1. 中断通知

  Linux 操作系统中大多数设备 I/O控制都采用中断处理方式

  中断处理控制方式需要有中断控制器的支持.

  > 中断控制器见`Linux内核完全注释修正版V5.0`的`2.4.1中断控制器`
1. DMA(direct memory access)

  用于 I/O 设备与系统内存之间进行批量数据传送, 整个操作过程需要使用专门的 DMA 控制器来进行而无需 CPU 插手. 由于在传输过程中无须软件介入，因此操作效率很高. 在 Linux 操作系统中，软盘驱动程序使用中断和 DMA 方式配合来实现数据的传输工作.
...
- 异步io
  ...
  向os发起io请求后os立即返回一个"已接受"信号, 此时发起者还可继续执行后续代码. 即每个IO请求，是否完成，都不会影响后续IO的发出. 如glibc的aio，以及linux的libaio, iouring等.

  > glibc的AIO本质是多线程模型, 开启新的线程以同步的方式来做I/O, 新的AIO辅助线程与发起AIO的线程以pthread_cond_signal()的形式进行线程间的同步.

  > libaio使用的是内核支持的异步I/O, 相较于glibc的减少了线程的负载和上下文切换.

- 阻塞/非阻塞io

  阻塞: 一直等到io操作完成为止

    进程行为: 若不能获得资源, 则挂起进程直到满足可操作的条件后再进行操作. 被挂起的进程进入睡眠状态, 被从调度器的运行队列移出, 直到等待的条件被满足.

    在linux驱动中, 可以使用等待队列来实现阻塞进程的唤醒.
  非阻塞: 调用操作io的接口后立即返回，但是并不保证IO操作成功, 此时外层需要一个循环来处理, 费cpu.

    进程行为: 进程在不能进程设备操作时, 并不会挂起, 它要么放弃, 要么不停地查询, 直到可进行操作为止
- direct io
