...
## 启动过程
假设芯片内部有SRAM, 起始地址是0x0, DDR内存的起始地址是0x80000000. SBI固件(如OpenSBI)运行在M模式, 起始地址是0x80000000, u-boot和linux内核运行在S模式, 起始地址是0x80200000. 芯片启动的一般流程:
1. 芯片上电, 运行BOOTROM
1. BOOTROM会初始化NorFlash等外部存储介质, 把OpenSBI加载到内存中, 并跳转到DDR内存中继续执行
	
	芯片内部的BOOTROM在复位上电阶段会把NOR Flash前4KB的代码加载到SRAM中. cpu上电复位, 从0x0开始取指执行前4KB的代码. 初始化DDR内存, 复制OpenSBI代码到DDR内存中, 此时运行地址是0x0, 链接地址是0x80000000, 它们不同. 再执行重定位, 跳转到DDR内存中, 此时运行地址和链接地址一致

1. OpenSBI切换到S模式, 并运行S模式下的u-boot
1. u-boot初始化硬件和启动环境, 跳转到linux内核并运行

	u-boot把内核镜像内容复制到0x80200000(_start运行地址), 并开始执行, 再内核重定位到0xFFFFFFE000000000(_start链接地址, 在内核虚拟地址)