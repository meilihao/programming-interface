# x86_64
[intel主板架构](http://img.expreview.com/news/2018/05/14/z3902.png)
...
数据单元包括 CPU 内部的缓存和寄存器组,空间很小,但是速度飞快,可以暂时存放数据和运算结果. 寄存器组可分为专用寄存器和通用寄存器. 专用寄存器的作用是固定的，分别寄存相应的数据；而通用寄存器用途广泛并可由程序员规定其用途
...
- ES, 扩展段寄存器
- SS, 栈寄存器(Stack Register) : 栈是程序运行中一个特殊的数据结构,数据的存取只能从一端进行,秉承后 进先出的原则. 凡是与函数调用相关的操作,都与栈紧密相关.
...
- ![real-mode memory map](http://www.qingpingshan.com/uploads/allimg/170627/10535A225-0.jpg)
...
> 以上这两个 bootloader 程序合在一起就构成了我们在 Linux 系统中经常用到的![GRUB2](https://zh.wikipedia.org/wiki/GNU_GRUB#/media/File:GNU_GRUB_components.svg)
