...
- [从零写OS内核-第三篇：Multiboot 协议详解 —— 内核与引导程序的“通用语言”](https://zhuanlan.zhihu.com/p/1969411942864097550)
...
始于 BIOS（Basic Input/Output System）, 当按下电源键：
1. CPU 从 地址 0xFFFFFFF0 开始执行（这是复位向量）
1. BIOS 代码（固化在主板 ROM 中）被加载运行
1. BIOS 自检硬件（内存、磁盘、键盘等）
1. 然后，BIOS 加载引导扇区（512 字节）到内存 0x7C00，并跳转执行

但现代系统很少直接用 BIOS 引导,  通常借助GRUB（一个更强大的 bootloader），它支持 Multiboot 协议，能直接加载 ELF 格式的内核

GRUB 的作用：
1. 解析内核 ELF 文件
1. 将代码段、数据段放到指定内存位置
1. 设置好 CPU 状态（已处于保护模式）
1. 跳转到 _start 入口
...
- [A20 Line](https://wiki.osdev.org/A20_Line)
    ...
    含多种尝试开启A20的汇编代码
- [A20 Line](http://wiki.foofun.cn/index.php?title=A20_Line)

    含多种尝试开启A20的汇编代码
- [关于A20](https://blog.csdn.net/longintchar/article/details/79365928)

A20 地址线问题是 x86 架构向后兼容性的一个历史遗留产物, 是一个特意设计的机制，目的是为了让 80286 处理器能在实模式下完美地运行为 8086 编写的程序.

如果段基址和段内偏移地址都为 16 位的最大值, 即 0xFFFF:0xFFFF, 最大
地址是 0xFFFF0+xFFFF=0x10FFEF=1M+64K-1-16. 由于实模式下的地址线是 20 位, 最大寻址空间是 1MB, 即0x00000~
0xFFFFF, 超出 IMB 内存的部分在逻辑上也是正常的, 但物理内存中却没有与之对应的部分. 为了让`段
基址:段内偏移地址`策略继续可用, CPU 采取的做法是将超过 1MB的部分自动回绕到 0 地址, 继续从 0
地址开始映射, 相当于把地址对1MB求模, 超过 1MB 多余出来的64K部分内存被称为高端内存区 HMA(High memory area). 这种策略就是地址环绕wrap-around.

地址回绕是为了兼容 8086/8088 的实模式(地址线20位, 最高可寻址 16MB), 80286为了实现对 8086 程序的向后兼容，80286 在其**实模式（Real Mode）**下必须模仿 8086 的行为, 因此Intel 引入了 A20 地址门（A20 Gate）. A20 Gate 是一个位于主板上的硬件开关，通常由键盘控制器（Keyboard Controller）控制.

当 A20 Gate 禁用时，地址线 A20 被强制为 0，无论处理器发出什么地址，A20 信号都无法到达内存控制器。这样，当程序访问 0x100000 时，由于 A20 被强制为 0，实际访问的地址是 0x00000，完美地模拟了 8086 的内存回卷特性. 当 A20 Gate 启用时，地址线 A20 正常工作, 程序可以访问超过 1 MB 的内存，进入 80286 的“高内存区”（High Memory Area, HMA）.

32位cpu(80386 及以上)实际有32根地址线, 处理地址环绕的方法:
- 如果 A20Gate 被打开, 当访问到 0x100000~0x10FFEF 之间的地址时, CPU 将真正访问这块物理内存
- 如果 A20Gate 被禁止, 当访问到 0x100000~Ox10FFEF 之间的地址时, CPU 将采用 8086/8088 的地址回绕

为了访问 1 MB 以上的内存，首先必须启用 A20 地址线.

接下来的代码将开启 A20(第21根地址线) 以开启实模式下的4GB寻址(使用32位寄存器)功能.
...
1. 通过键盘控制器（最常用）
    ...
    .wait_send_command:
        in al, dx
        test al, 0x02  ; Check if input buffer is full (bit 1)
        jnz .wait_send_command

    ; Send "write command" instruction (0xD1)
    mov al, 0xD1
    out dx, al
    ...
    ; Send "Enable A20 Gate" command (0xDF)
    mov dx, 0x60
    mov al, 0xDF
    out dx, al
    ```
1. A20快速门 ( Fast Gate A20, 一些较新的芯片组提供了更直接、更快速的启用 A20 Gate 的方法),它使用 I/O端口 Ox92来处理A20信号线. 对于不含键盘控制器的操作系统,就只能使用 Ox92端口来控制,但是该端口有可能被其他设备使用.
    ...
    这种方法更快，但在较老的硬件上可能不可用。现代的引导加载程序（如 GRUB、LILO）通常会尝试这两种方法，以确保在各种硬件上都能成功启用 A20
1. 使用 BIOS 中 断服务程序INT 15h的 主功能号AX=2401 可开启 A20地址线 ,功能号AX=2400可禁; 用A20地址线,功能号AX=2403 可查询A20地址线的当前状态
    ...
    jc error ; CF 置位表示出错
    ```
1. 通过读Oxee端口来开启 A20信号线,而写该端口则会禁止A20信号线

   ```
   enable_a20:
    in al, 0xee   ; 读取端口0xee：快速开启A20（硬件支持时有效）
    ret           ; 返回
   ```

   该方法简单且无需等待，但并非所有硬件都支持（较老的系统可能无效）。若失效，需回退到其他方法（如键盘控制器或BIOS中断）.

为了兼容地址回绕这个特性，后来生产的CPU都会默认关闭第21根地址线，需要操作系统在进入保护模式前手动开启. 不过某些BIOS, 包括模拟器（qemu, Bochs等）会默认打开A20地址线, 不需要手动开启.

在实模式下使用 32 位寄存器访问 4 GB 内存，是 x86 处理器的一个“小把戏”。它利用了 32 位处理器的硬件特性，通过指令前缀和 A20 地址线绕过了实模式的寻址限制。它只是为了向后兼容和过渡而存在，而非现代操作系统的常规寻址方式. 同时这种技巧通常只在引导加载程序或特殊的 DOS 扩展程序（如 DOS4GW）中使用. 现代操作系统内核（如 Linux 或 Windows）在启动时会立即切换到保护模式，因此它们不会使用这种方法.
